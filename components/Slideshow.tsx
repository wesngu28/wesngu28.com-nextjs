            ...
            {photos.map((photo: StaticImageData, i: number) => {
                return (
                    <div key={i} className={`flex flex-col justify-center ${photos.length === 6  ? 'h-96 md:h-1/2 w-[200px] md:w-1/2' : null } m-auto relative ${active === i ? 'block animate-slideshow' : 'hidden'}`}>
                        {caption ? 
                            <div>
                                <h2 className="text-center p-4 text-sm md:text-md font-bold">{caption[i].location}</h2>
                                <p className="text-center text-sm md:text-md mb-4" ><span className="font-bold">{caption[i].title}</span> - {caption[i].duration}</p>
                            </div>
                        : null}

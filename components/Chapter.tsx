...
import target from '../public/map/target.png'
...
const Chapter = forwardRef<HTMLDivElement, Props>(({ index, view }: Props, ref) => {
    ...
    return (
        <div ref={ref} className={`${view ? 'opacity-100 transition-opacity duration-1000' : 'opacity-25'} rounded-3xl bg-[#1F2022] text-white z-10 absolute mb-11 ${chapters[index].top[1]} ${chapters[index].top[0]} md:p-4 md:w-1/4 h-max rounded overflow-scroll`}>
            ...
            {chapters[index].source ? <div className="p-4 flex justify-center"><Image alt={chapters[index].alt} src={chapters[index].source!} /></div> : null}
            <div>
                ...
                {(chapters[index].top[1] === 'top-[930vh]') || (chapters[index].top[0] === 'md:top-[630vh]') ? <div>
                    <Slideshow photos={[nhstc, target, fresh]} caption={[
                        { location: 'Newport Hills Swim Club', duration: 'Mar 2018 - Sep 2020', title: 'Lifeguard' },
